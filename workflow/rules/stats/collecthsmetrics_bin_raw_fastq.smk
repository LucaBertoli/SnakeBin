...
def near_distance(wildcards):
    with open(config['results_folder'] + "/merged_raw_fastq/split_bam_merged/" + "insert_" + wildcards.bin_start + "-" + wildcards.bin_end + "/alignment.dedup.recal.clip.output", newline='') as csvfile:
...
rule hsmetrics_bin_raw_fastq:
    ...
    output:
        output_metrics=config['results_folder']+"/merged_raw_fastq/split_bam_merged/insert_{bin_start}-{bin_end}/alignment.dedup.recal.clip_HsMetrics.txt",
        output_target=config['results_folder']+"/merged_raw_fastq/split_bam_merged/insert_{bin_start}-{bin_end}/alignment.dedup.recal.clip_PER_TARGET_COVERAGE.txt",
        output_base=config['results_folder']+"/merged_raw_fastq/split_bam_merged/insert_{bin_start}-{bin_end}/alignment.dedup.recal.clip_PER_BASE_COVERAGE.txt",
    params:
