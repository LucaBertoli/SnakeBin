...
rule bwa_mem2_mem_merged_fastq_from_samples:
    input:
        merged_fastq_R1=config['results_folder']+"/merged_downsampled_bam/{million_fragment_downsampling}/insert_{bin_start}-{bin_end}/merged.R1.fastq.gz",
        merged_fastq_R2=config['results_folder']+"/merged_downsampled_bam/{million_fragment_downsampling}/insert_{bin_start}-{bin_end}/merged.R2.fastq.gz",
    output:
        start_sorted_bam=temp(config['results_folder']+"/merged_downsampled_bam/{million_fragment_downsampling}/insert_{bin_start}-{bin_end}/start_sorted.bam"), #bwa-mapped samples
    threads: 
...
rule samtools_index_merged_fastq_from_samples:
    ...
    output:
        start_sorted_bam_bai=temp(config['results_folder']+"/merged_downsampled_bam/{million_fragment_downsampling}/insert_{bin_start}-{bin_end}/start_sorted.bam.bai"),
    threads: 
