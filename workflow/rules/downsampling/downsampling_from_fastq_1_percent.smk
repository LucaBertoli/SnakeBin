...
def num_farg(samples):
    ...
    return int(1000000 // len(samples))
...
rule extract_reads_from_fastq:
    input:
        ...
        map_clip_flagstat=config['results_folder']+"/{sample}/split_fastq/
    output:
        downsampled_fastq_R1=temp(config['results_folder']+"/{sample}/downsampled_fastq_1_perc/fastq_{bin_start}-{bin_end}/{sample}_R1.fastq.gz"),
        downsampled_fastq_R2=temp(config['results_folder']+"/{sample}/downsampled_fastq_1_perc/fastq_{bin_start}-{bin_end}/{sample}_R2.fastq.gz"),
    threads: 
